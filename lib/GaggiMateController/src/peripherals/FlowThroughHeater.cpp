...
constexpr float TUNER_HEATER_OUTPUT_SPAN = 500.0f; // this is the relay duty cycle 500ms is 100% duty cycle
...
void FlowThroughHeater::setup() {
    ...
    setupPid();
    xTaskCreate(loopTask, "FlowThroughHeater::loop", configMINIMAL_STACK_SIZE * 4, this, TASK_PRIO_REGULATORS, &taskHandle);
}

void FlowThroughHeater::setupPid() {

    flowThroughPID->setSamplingFrequency(10.0f); //Hz
    flowThroughPID->setCtrlOutputLimits(0.0f, TUNER_HEATER_OUTPUT_SPAN);
...
void FlowThroughHeater::setRegulatorReset() {
    flowThroughPID->reset();
}
...
void FlowThroughHeater::loopPid() {
    softPwm(TUNER_HEATER_OUTPUT_SPAN);
    temperature = sensor_temperature->read();
    flow = sensor_flow->read();
...
void FlowThroughHeater::loopTask(void *arg) {
    ...
    while (true) {
        heater->loop();
        vTaskDelay(FLOW_REGULATOR_LOOP_INTERVAL_MS / portTICK_PERIOD_MS);
    }
}